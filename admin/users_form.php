<?php
    ...
    if($_POST)
    {
        ...
        if(!empty($_FILES['product_picture']['name'])) // I'm checking if I got a result for the 1st picture
        {
            ...
            $picture_name = $_POST['pseudo'] . '-' . rand(1,999) .  '_' . $_FILES['product_picture']['name'];
        ...
        if(empty($msg_error))
        {
            ...
            {
                $result = $pdo->prepare("UPDATE user SET user=:user, pseudo=:pseudo, pwd=:pwd, firstname=:firstname, lastname=:lastname, email=:email, gender=:gender, city=:city, zip_code=:zip:_code, address=:address, privilege=:privilege, picture=:picture WHERE user = :user");

                $result->bindValue(':user', $_POST['user'], PDO::PARAM_INT);
            }
    ...
    $user = (isset($update_user)) ? $update_user['user'] : '';
